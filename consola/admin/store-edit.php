...
if((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "produtos_stock_form")) {
  ...
  $query_rsList = "SELECT * FROM l_pecas_store WHERE l_pecas_store.product_id=:id ORDER BY l_pecas_store.id DESC";
...
$totalRows_rsList = $rsList->rowCount();
  ...
  if($totalRows_rsTamanhos>0) {
    if(($car1 > 0 && $op1 == 0) || ($car2 > 0 && $op2 == 0) || ($car3 > 0 && $op3 == 0) || ($car4 > 0 && $op4 == 0) || ($car5 > 0 && $op5 == 0))
      ...
      /*   $query_rs_Check = "SELECT * FROM l_pecas_store";
        $rsProcCheck = DB::getInstance()->prepare($query_rs_Check);
        $rsProcCheck->execute();
        $row_rsCheck = $rsProcCheck->fetchAll(PDO::FETCH_ASSOC);
        if($row_rsCheck["product_id"] == $id AND $row_rsCheck["b_name_pro"] != $namemulti)
        {*/
          $insertSQL = "INSERT INTO l_pecas_store (product_id, b_name_pro, phone) VALUES (:max_id, :b_name , :phone)";
          ...
          if(!$manter) 
            ...
            header("Location: store-edit.php?alt=1&id=".$id);
       // }
      }
...
if((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "produtos_stock_atualiza")) {
  ...
  $query_rsInferior = "SELECT * FROM l_pecas_store WHERE product_id=:id";
  ...
  $query_rsSuperior = "SELECT * FROM l_pecas_store WHERE product_id=:id";
 ...
 // $minimo=$row_rsInferior['MIN(l_pecas_store.id)'];
  //$maximo=$row_rsSuperior['MAX(l_pecas_store.id)'];
  
 // for($i=$minimo; $i<=$maximo; $i++) {name


  $query_rsSuperior = "SELECT MAX(l_pecas_store.id) FROM l_pecas_store WHERE product_id=:id";
  $rsSuperior = DB::getInstance()->prepare($query_rsSuperior);
  $rsSuperior->bindParam(':id', $id, PDO::PARAM_INT);
  $rsSuperior->execute();
  $row_rsSuperior = $rsSuperior->fetch(PDO::FETCH_ASSOC);
  ...
  for($i=$minimo; $i<=$maximo; $i++) {
    ...
    if(isset($phone)) {  
          //$store_idm = $store_id[$i];

          $insertSQL = "UPDATE l_pecas_store SET phone=:phone, primery=:primery WHERE id=:i";
          $rsInsert = DB::getInstance()->prepare($insertSQL);
          $rsInsert->bindParam(':phone', $phone, PDO::PARAM_STR, 5);   
          ...
          if(!$manter_act) 
            ...
            header("Location: store-edit.php?alt=2&id=".$id);
...
if(isset($_GET['reg'])) { 
  if(isset($_GET['rem']) && $_GET['rem']==1) {
    ...
    $insertSQL = "DELETE FROM l_pecas_store WHERE id=:projecto AND product_id=:id";
    $rsInsert = DB::getInstance()->prepare($insertSQL);
    ...
    header("Location: store-edit.php?id=".$id."&r=1");
  }
}
...
$query_rsTamanhos = "SELECT * FROM l_pecas_store WHERE l_pecas_store.product_id=:id ORDER BY l_pecas_store.product_id DESC";
$rsTamanhos = DB::getInstance()->prepare($query_rsTamanhos);
$rsTamanhos->bindParam(':id', $id, PDO::PARAM_INT);
$rsTamanhos->execute();
$row_rsTamanhos = $rsTamanhos->fetch(PDO::FETCH_ASSOC);
              ...
              <div class="portlet-body">
                <div class="tabbable">
                  <ul class="nav nav-tabs">
                    ...
                    <li class="nav-tab active" onClick="window.location='store-edit.php?id=<?php echo $id; ?>'"> <a href="javascript:void(null)" data-toggle="tab"> Store Locater </a> </li>
                   ...
                   <!--  <li class="nav-tab" onClick="window.location='produtos-edit-quantidades.php?id=<?php echo $id; ?>'"> <a href="javascript:void(null)" data-toggle="tab"> <?php echo $RecursosCons->RecursosCons['tab_quantidades']; ?> </a> </li> -->
                    ...
                    <li class="nav-tab" onClick="window.location='store-edit.php?id=<?php echo $id; ?>'"> <a href="javascript:void(null)" data-toggle="tab"> Supplier </a> </li>
                          ...
                          //if($totalRows_rsCat>0) { ?>
                            <div class="form-group">
                              <?php /*$count=0; while($row_rsCat = $rsCat->fetch()) { 
                                ...
                                $query_rsCat2 = "SELECT * FROM store_locater";
                                $rsCat2 = DB::getInstance()->prepare($query_rsCat2);
                                $rsCat2->execute();
                                $totalRows_rsCat2 = $rsCat2->rowCount();
                                DB::close();

                                $query_rsCat = "SELECT * FROM store_locater";
                                $rsCat = DB::getInstance()->prepare($query_rsCat);
                                $rsCat->execute();
                                $totalRows_rsCat = $rsCat->rowCount();
                                ...
                                if($totalRows_rsCat>0) { ?> 
                                  ...
                                  <div style="padding-bottom: 10px" class="col-md-10">
                                    <select multiple class="form-control select2me" id="b_name" name="b_name[]" <?php //if($row_rsCat['id']!=$car1 && $row_rsCat['id']!=$car2 && $row_rsCat['id']!=$car3 && $row_rsCat['id']!=$car4 && $row_rsCat['id']!=$car5 && $totalRows_rsTamanhos>0) echo "disabled";?>>
                                      <option value="0" disabled>Selecionar </option>                 
                                      <?php foreach ($row_rsCat as $row_rsCat2) { ?>
                                        <option value="<?php echo $row_rsCat2['id']; ?>"><?php echo $row_rsCat2['b_name']; ?></option>
                                      <?php } ?>
                                    </select>
                                  </div>
                                <?php } else { ?>
                  ...
                  <div class="table-scrollable">
                    <table class="table table-hover">
                      ...
                      <tbody>
                        ...
                        <?php 
                          $query_rsCat3 = "SELECT * FROM store_locater WHERE id = ".$row_rsList['b_name_pro'];
                          $rslist = DB::getInstance()->prepare($query_rsCat3);
                          $rslist->execute();
                        ...
                        <tr>
                          <td><?php echo $cont; ?></td>
                          <td><?php echo $row_rslist["b_name"] ?>
                          </td>

                          <td><input type="text" class="form-control col-md-1" name="phone[]" id="phone[]" value="<?php echo $row_rslist["phone"]; ?>" disabled></td>  
                     
                         <td>
                        <input type="checkbox" <?php if($row_rsList['primery'] == 1) { echo "checked"; } ?> name="switch" id="switch_<?php echo $row_rsList['id']; ?>" class="make-switch" data-on-color="success" data-on-text="<?php echo $RecursosCons->RecursosCons['text_visivel_sim']; ?>" data-off-color="danger" data-off-text="<?php echo $RecursosCons->RecursosCons['text_visivel_nao']; ?>" data-off-value="0" data-on-value="1">
                            ...
                            <div class="modal fade" id="modal_delete_<?php echo $row_rsList['id'];?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                              <div class="modal-dialog">
                                <div class="modal-content">
                                  ...
                                  <div class="modal-body"> <?php echo $RecursosCons->RecursosCons['msg_elimina_registo']; ?> </div>
                                  <div class="modal-footer">
                                    <button type="button" class="btn blue" onClick="document.location='store-edit.php?id=<?php echo $id; ?>&rem=1&reg=<?php echo $row_rsList['id'];?>'"><?php echo $RecursosCons->RecursosCons['txt_ok']; ?></button>
                                    <button type="button" class="btn default" data-dismiss="modal"><?php echo $RecursosCons->RecursosCons['txt_cancelar']; ?></button>
                                  </div>
                                </div>
                                <!-- /.modal-content --> 
...
jQuery(document).ready(function() {    
   ...
   FormValidation.init();
    ...
    $.ajax({
      ...
      type: 'POST',
      data: {op: 'store_visivel', valor: valor, id: id},
    })
    .done(function(data) {

...
<?php
    ...
    $records["customActionStatus"] = "OK"; // pass custom message(useful for getting status of group actions)
		...
		if($opcao == 3 || $opcao == 4) { // colocar os destaques delecionados visíveis
			foreach ($row_rsLinguas as $linguas) {
				if($opcao == 3) $query_rsUpd = "UPDATE homepage_image_".$linguas["sufixo"]." SET visivel = '1' WHERE id IN $lista";
				else $query_rsUpd = "UPDATE homepage_image_".$linguas["sufixo"]." SET visivel = '0' WHERE id IN $lista";
		...
		if($opcao == 5 || $opcao == 6) { // colocar os destaques delecionados visíveis
			foreach ($row_rsLinguas as $linguas) {
				if($opcao == 5) $query_rsUpd = "UPDATE homepage_image_".$linguas["sufixo"]." SET destacado = '1' WHERE id IN $lista";
				else $query_rsUpd = "UPDATE homepage_image_".$linguas["sufixo"]." SET destacado = '0' WHERE id IN $lista";
				$rsUpd = DB::getInstance()->prepare($query_rsUpd);
				$rsUpd->execute();
			}
		}
			...
			foreach ($row_rsLinguas as $linguas) {
				$query_rsP = "SELECT * FROM homepage_image_".$linguas['sufixo']." WHERE id IN $lista";
				...
				$query_rsDel = "DELETE FROM homepage_image_".$linguas['sufixo']." WHERE id IN $lista";
 ...
 /*  $sOrder = " ORDER BY ordem ASC, data DESC";
  ...
  $query_rsTotal = "SELECT id, nome, imagem ,titulo , link FROM homepage_image".$extensao." WHERE id > '0'".$where_pesq;
  ...
  $query_rsTotal = "SELECT * FROM homepage_image".$extensao."  WHERE id > '0'".$where_pesq." LIMIT $iDisplayStart, $iDisplayLength";
  $rsTotal = DB::getInstance()->prepare($query_rsTotal);
  $rsTotal->execute();
  $totalRows_rsTotal = $rsTotal->rowCount();
  ...
  while($i < $end && $row_rsTotal = $rsTotal->fetch()) {
	  ...
	  $titulo = utf8_encode($row_rsTotal['titulo']);
	  $link = utf8_encode($row_rsTotal['link']);
	
	  if($row_rsTotal['imagem'] && file_exists("../../../imgs/homepage/".$row_rsTotal['imagem'])) $imagem = '<a href="../../../imgs/homepage/'.utf8_encode($row_rsTotal['imagem']).'" data-fancybox="gallery" data-caption="'.utf8_encode($row_rsTotal['nome']).'" ><img src="../../../imgs/homepage/'.utf8_encode($row_rsTotal['imagem']).'" height="70" /></a>';
    ...
    $records["data"][] = array(
	  ...
	  '<a href="image-edit.php?id='.$id.'" class="btn btn-xs default btn-editable"><i class="fa fa-pencil"></i> '. $RecursosCons->RecursosCons['btn_editar'].'</a>',
    );

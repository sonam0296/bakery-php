...
<?php 
    ...
    $totalRows_rsLinguas = $rsLinguas->rowCount();
		...
		if($opcao == 3 || $opcao == 4) { // colocar todas as notícias visíveis
			foreach($row_rsLinguas as $linguas) { 
				if($opcao == 3) $query_rsUpd = "UPDATE supplier SET visivel = '1' WHERE id IN $lista";
				else $query_rsUpd = "UPDATE supplier SET visivel = '0' WHERE id IN $lista";
			...
			foreach($row_rsLinguas as $linguas) { 
				...
				$query_rsDel = "DELETE FROM supplier WHERE id IN $lista";
  ...
  $colunas = array( '', 'title', 'data', 'product_order_number', 'ordem', 'visivel', '');
  ...
  if(isset($_REQUEST['action']) && $_REQUEST['action']=="filter") {
		...
		if($pesq_nome != "") $where_pesq .= " AND (title = '$pesq_nome' OR title LIKE '%$pesq_nome%')";
  ...
  $query_rsTotal = "SELECT * FROM supplier WHERE id > '0'".$where_pesq.$sOrder;
  ...
  $query_rsTotal = "SELECT * FROM supplier WHERE id > '0'".$where_pesq.$sOrder." LIMIT $iDisplayStart, $iDisplayLength";
  $rsTotal = DB::getInstance()->query($query_rsTotal);
  $rsTotal->execute();
  $totalRows_rsTotal = $rsTotal->rowCount();
  ...
  while($i < $end && $row_rsTotal = $rsTotal->fetch()) {
    ...
    $product_order_number = $row_rsTotal['product_order_number'];
    ...
    $records["data"][] = array(
	  ...
	  '<a href="supplier-edit.php?id='.$id.'" class="btn btn-xs default btn-editable"><i class="fa fa-pencil"></i> '. $RecursosCons->RecursosCons['btn_editar'].'</a>',
    );

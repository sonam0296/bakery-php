...
if((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "caract_opcao")) {
	$manter = $_POST['manter'];
	
	if($_POST['nome']!='') {
		$insertSQL = "UPDATE l_caract_opcoes_en SET nome=:nome WHERE id=:id";
    ...
    if($_POST['categoria'] != 1)
      $cor = NULL;
		...
		while($row_rsLinguas = $rsLinguas->fetch()) {	
			$insertSQL = "UPDATE l_caract_opcoes_".$row_rsLinguas["sufixo"]." SET categoria=:categoria, cor=:cor WHERE id=:id";
			$rsInsert = DB::getInstance()->prepare($insertSQL);
			$rsInsert->bindParam(':categoria', $_POST['categoria'], PDO::PARAM_INT, 5);	
		...
		if(!$manter)
			header("Location: caract_opcoes.php?alt=1");
		else
			header("Location: caract_opcoes-edit.php?id=".$id."&alt=1");
	}
}

$query_rsP = "SELECT id, nome, categoria, cor FROM l_caract_opcoes_en WHERE id=:id";
...
$query_rsCat = "SELECT id, nome FROM l_caract_categorias_en ORDER BY ordem ASC, nome ASC";
...
<div class="page-container">
  ...
  <div class="page-content-wrapper">
    <div class="page-content"> 
      <!-- BEGIN PAGE HEADER-->
      <h3 class="page-title"><?php echo $RecursosCons->RecursosCons['caracteristicas']; ?> <small><?php echo $RecursosCons->RecursosCons['editar_registo']; ?></small> </h3>
      <div class="page-bar">
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-footer">
              <button type="button" class="btn blue" onClick="document.location='caract_opcoes.php?rem=1&id=<?php echo $row_rsP["id"]; ?>'"><?php echo $RecursosCons->RecursosCons['txt_ok']; ?></button>
      ...
      <div class="row">
        <div class="col-md-12">    
          ...
          <form id="caract_opcao" name="caract_opcao" class="form-horizontal form-row-seperated" method="post" role="form" enctype="multipart/form-data">
            <input type="hidden" name="manter" id="manter" value="0">
            <div class="portlet">
              <div class="portlet-title">
                <div class="caption"> <i class="fa fa-pencil-square"></i><?php echo $RecursosCons->RecursosCons['caracteristicas']; ?> - <?php echo $row_rsP['nome']; ?></div>
                <div class="form-actions actions btn-set">
                  <button type="button" name="back" class="btn default" onClick="document.location='caract_opcoes.php'"><i class="fa fa-angle-left"></i> <?php echo $RecursosCons->RecursosCons['voltar']; ?></button>
              ...
              <div class="portlet-body">
                <div class="form-body">
                  ...
                  <!-- <div id="div_cor" class="form-group">
                    <label class="col-md-2 control-label" for="cor">Cor:</label>
                    <div class="col-md-8">
                    	<div style="display:table-cell;"><input type="text" class="form-control color" name="cor" id="cor" value="<?php echo $row_rsP['cor']; ?>" style="width:100px"></div>
                      <div style="display:table-cell;padding-left:10px;vertical-align:middle"><i class="icon-trash" onclick="document.getElementById('cor').value='';document.getElementById('cor').style.backgroundColor='#FFFFFF'" style="cursor:pointer"></i></div>                      
                    </div>
                  </div> -->         
                </div>
              </div>
            </div>
            <input type="hidden" name="MM_insert" value="caract_opcao" />
          </form>
        </div>
      </div>
      <!-- END PAGE CONTENT--> 

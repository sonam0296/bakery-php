...
if((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "frm_faqs")) {
	...
	$query_rsP = "SELECT * FROM faq_content".$extensao;
	...
	if($totalRows_rsP == '0' && $_POST['pergunta']!='')
	{
		...
		while($row_rsLinguas = $rsLinguas->fetch()) {	
			...
			$insertSQL = "INSERT INTO faq_content_".$row_rsLinguas["sufixo"]." ( id , pergunta, resposta) VALUES ( '1', :pergunta, :resposta)";
	...
	} else if( $_POST['pergunta']!='')
	{
		
		$insertSQL = "UPDATE faq_content".$extensao." SET pergunta=:pergunta, resposta=:resposta WHERE id=1";
		$rsInsert = DB::getInstance()->prepare($insertSQL);
		$rsInsert->bindParam(':pergunta', $_POST['pergunta'], PDO::PARAM_STR, 5);	
		$rsInsert->bindParam(':resposta', $_POST['resposta'], PDO::PARAM_STR, 5);	
		$rsInsert->execute();
	...
	if($_POST['pergunta']!=''  && $tab_sel == 1) {
		/* $categoria = $_POST['categoria'];
		
		$insertSQL = "UPDATE faq_content".$extensao." SET pergunta=:pergunta, resposta=:resposta WHERE id=:id";
		$rsInsert = DB::getInstance()->prepare($insertSQL);
		$rsInsert->bindParam(':pergunta', $_POST['pergunta'], PDO::PARAM_STR, 5);	
		$rsInsert->bindParam(':resposta', $_POST['resposta'], PDO::PARAM_STR, 5);	
		...
		if($totalRows_rsLinguas > 0) {
			while($row_rsLinguas = $rsLinguas->fetch()) {
				$insertSQL = "UPDATE faq_content".$row_rsLinguas['sufixo']." SET categoria=:categoria WHERE id=:id";
				$rsInsert = DB::getInstance()->prepare($insertSQL);
		...
		if(!$manter) 
			header("Location: faqs.php?alt=1");
		else
			header("Location: faqs-content.php?id=".$id."&alt=1&tab_sel=1");
	}
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-footer">
              <button type="button" class="btn blue" onClick="document.location='faqs.php?rem=1&id=<?php echo $row_rsP["id"]; ?>'"><?php echo $RecursosCons->RecursosCons['txt_ok']; ?> </button>
          ...
          <form id="frm_faqs" name="frm_faqs" class="form-horizontal form-row-seperated" method="post" role="form" enctype="multipart/form-data">
            ...
            <div class="portlet">
              <div class="portlet-title">
                <div class="caption"> <i class="fa fa-pencil-square"></i><?php echo $RecursosCons->RecursosCons['faqs']; ?> - <?php echo $row_rsP["pergunta"]; ?> </div>
                ...
                <div class="form-body">
                  ...
                  <?php } ?>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-8">
                        <input type="text" class="form-control" name="pergunta" id="pergunta" value="<?php echo $row_rsP['pergunta']; ?>" data-required="1">
                      </div>
                    </div>
                    <div class="form-group">
                      <label class="col-md-2 control-label" for="resposta"><?php echo $RecursosCons->RecursosCons['resposta_label']; ?>: </label>
                      <div class="col-md-8">
                        <textarea class="form-control" id="resposta" name="resposta"><?php echo $row_rsP['resposta']; ?></textarea>
                      </div>
                    </div>
                  </div>
                </div>
...
<script src="<?php echo ROOTPATH_HTTP_CONSOLA; ?>assets/global/plugins/jcrop/js/jquery.Jcrop.min.js"></script> 
<script src="<?php echo ROOTPATH_HTTP_CONSOLA; ?>assets/admin/pages/scripts/form-image-crop.js"></script> 

...
if((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "filt_categ")) {
	if($_POST['nome']!='') {
		$insertSQL = "SELECT MAX(id) FROM l_filt_categorias_$lingua_consola";
    ...
    $totalRows_rsLinguas = $rsLinguas->rowCount();
		...
		while($row_rsLinguas = $rsLinguas->fetch()) {	
			$insertSQL = "INSERT INTO l_filt_categorias_".$row_rsLinguas["sufixo"]." (nome) VALUES (:nome)";
			$rsInsert = DB::getInstance()->prepare($insertSQL);
    ...
    DB::close();
		...
		header("Location: filt_categorias.php?env=1");
	}
}

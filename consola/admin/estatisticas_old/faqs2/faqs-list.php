...
<?php
    ...
    $records["customActionStatus"] = "OK"; // pass custom message(useful for getting status of group actions)
		...
		if($opcao == 3 || $opcao == 4) { // colocar todas as notícias visíveis
			foreach ($row_rsLinguas as $linguas) {
				if($opcao == 3) $query_rsUpd = "UPDATE faqs2_".$linguas["sufixo"]." SET visivel = '1' WHERE id IN $lista";
				else $query_rsUpd = "UPDATE faqs2_".$linguas["sufixo"]." SET visivel = '0' WHERE id IN $lista";
			...
			foreach ($row_rsLinguas as $linguas) {
				$query_rsDel = "DELETE FROM faqs2_".$linguas["sufixo"]." WHERE id IN $lista";
  ...
  $query_rsTotal = "SELECT * FROM faqs2_pt WHERE id > '0'".$where_pesq.$sOrder;
  ...
  $query_rsTotal = "SELECT * FROM faqs2_pt WHERE id > '0'".$where_pesq.$sOrder." LIMIT $iDisplayStart, $iDisplayLength";
  $rsTotal = DB::getInstance()->prepare($query_rsTotal);
  $rsTotal->execute();
  $totalRows_rsTotal = $rsTotal->rowCount();
  ...
  while($i < $end && $row_rsTotal = $rsTotal->fetch()) {
    ...
    $categoria = $row_rsTotal['categoria'];
		...
		$query_rsCat = "SELECT nome FROM faqs2_categorias_pt WHERE id =:categoria";

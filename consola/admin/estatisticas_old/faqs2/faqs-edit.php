...
if((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "frm_faqs")) {
	...
	$query_rsP = "SELECT * FROM faqs".$extensao." WHERE id=:id";
	...
	if($_POST['pergunta']!='' && $_POST['categoria'] != '' && $tab_sel == 1) {
		$categoria = $_POST['categoria'];
		
		$insertSQL = "UPDATE faqs".$extensao." SET pergunta=:pergunta, resposta=:resposta WHERE id=:id";
		...
		if($totalRows_rsLinguas > 0) {
			while($row_rsLinguas = $rsLinguas->fetch()) {
				$insertSQL = "UPDATE faqs2_".$row_rsLinguas['sufixo']." SET categoria=:categoria WHERE id=:id";
				$rsInsert = DB::getInstance()->prepare($insertSQL);
		...
		if(!$manter) 
			...
			header("Location: faqs-edit.php?id=".$id."&alt=1&tab_sel=1");
	}
}
...
$query_rsCats = "SELECT id, nome FROM faqs2_categorias_pt WHERE visivel = '1' ORDER BY nome ASC";
                      ...
                      <!-- <div class="form-group">
                          ...
                          <div class="col-md-8">
                            <select name="categoria" id="categoria" class="form-control select2me">
                              ...
                              <option value="<?php echo $row_rsCats["id"]; ?>" <?php if($row_rsP["categoria"] == $row_rsCats["id"]) { ?>selected<?php } ?>><?php echo $row_rsCats["nome"]; ?></option>
                              <?php } ?>
                            </select>
                          </div>
                        </div> -->

...
if(isset($_GET['rem']) && $_GET['rem'] == 1) {
    ...
    $totalRows_rsLinguas = $rsLinguas->rowCount();
		...
		while($row_rsLinguas = $rsLinguas->fetch()) {
			$query_rsDel = "UPDATE faq2_".$row_rsLinguas["sufixo"]." SET categoria = '0' WHERE categoria=:id";
			$rsDel = DB::getInstance()->prepare($query_rsDel);
			$rsDel->bindParam(':id', $_GET['id'], PDO::PARAM_INT);	
			$rsDel->execute();
		
			$query_rsDel = "DELETE FROM faq2_categorias_".$row_rsLinguas["sufixo"]." WHERE id=:id";
			$rsDel = DB::getInstance()->prepare($query_rsDel);
			$rsDel->bindParam(':id', $_GET['id'], PDO::PARAM_INT);	
			$rsDel->execute();
		}

...
<?php
    ...
    $records["customActionStatus"] = "OK"; // pass custom message(useful for getting status of group actions)
		...
		if($opcao == 3 || $opcao == 4) { // colocar todas as notícias visíveis
			foreach ($row_rsLinguas as $linguas) {
				if($opcao == 3) $query_rsUpd = "UPDATE faqs2_categorias_".$linguas["sufixo"]." SET visivel = '1' WHERE id IN $lista";
				else $query_rsUpd = "UPDATE faqs2_categorias_".$linguas["sufixo"]." SET visivel = '0' WHERE id IN $lista";
			...
			foreach ($row_rsLinguas as $linguas) {
				$query_rsDel = "UPDATE faqs2_".$linguas["sufixo"]." SET categoria = '0' WHERE categoria IN $lista";
				$rsDel = DB::getInstance()->prepare($query_rsDel);
				$rsDel->execute();
			
				$query_rsDel = "DELETE FROM faqs2_categorias_".$linguas["sufixo"]." WHERE id IN $lista";
				$rsDel = DB::getInstance()->prepare($query_rsDel);
				$rsDel->execute();
			}
		}
  ...
  $query_rsTotal = "SELECT id, nome, visivel, ordem FROM faqs2_categorias_pt WHERE id > '0'".$where_pesq.$sOrder;
  ...
  $query_rsTotal = "SELECT id, nome, visivel, ordem FROM faqs2_categorias_pt WHERE id > '0'".$where_pesq.$sOrder." LIMIT $iDisplayStart, $iDisplayLength";
  $rsTotal = DB::getInstance()->prepare($query_rsTotal);
  $rsTotal->execute();
  $totalRows_rsTotal = $rsTotal->rowCount();

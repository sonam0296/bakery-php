	...
	foreach ($array as $valor) {
		...
		if($campo_id>0 && $ordem_final>0) {				
			foreach($row_rsLinguas as $linguas) {			
				$insertSQL = "UPDATE banners_h_".$linguas["sufixo"]." SET ordem=:ordem_final WHERE id=:campo_id";
...
if($_POST['op'] == 'banners_visivel') {
	$id = $_POST['id'];
	$valor = $_POST['valor'];

  foreach($row_rsLinguas as $linguas) {		
		$insertSQL = "UPDATE banners_h_".$linguas["sufixo"]." SET visivel=:visivel WHERE id=:id";
		$rsInsert = DB::getInstance()->prepare($insertSQL);

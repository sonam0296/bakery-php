...
if(isset($_GET['rem']) && $_GET['rem'] == 1) {
	if(isset($_GET['id']) && $_GET['id'] != "" && $_GET['id'] != 0) {
		...
		$deleteSQL = "DELETE FROM transportadoras WHERE id = :id";
		$rs_deleteSQL = DB::getInstance()->prepare($deleteSQL);
		$rs_deleteSQL->bindParam(':id', $id, PDO::PARAM_INT);
		$rs_deleteSQL->execute();
		
		$deleteSQL = "DELETE FROM transp_valores WHERE id_transp = :id";
		$rs_deleteSQL = DB::getInstance()->prepare($deleteSQL);
		$rs_deleteSQL->bindParam(':id', $id, PDO::PARAM_INT);
		$rs_deleteSQL->execute();
		
		$updateSQL = "UPDATE zonas_met_envio SET tabela='0' WHERE tabela=:id";
		$rs_updateSQL = DB::getInstance()->prepare($updateSQL);
		$rs_updateSQL->bindParam(':id', $id, PDO::PARAM_INT);
		$rs_updateSQL->execute();
    
		DB::close();
		
		header("Location: p_tab_transportadoras.php?r=1");
	}
}
        ...
        <div class="col-md-12"> 
          ...
          <div class="portlet">
            <div class="portlet-title">
              ...
              <div class="actions"> <a href="p_tab_transportadoras-insert.php" class="btn default yellow-stripe"> <i class="fa fa-plus"></i> <span class="hidden-480"> <?php echo $RecursosCons->RecursosCons['novo_registo']; ?>  </span> </a> </div>
            </div>
...
<script src="p_tab_transportadoras-rpc.js" data-texto1="<?php echo $RecursosCons->RecursosCons['a_carregar']; ?>" data-texto2="<?php echo $RecursosCons->RecursosCons['selec_opcao']; ?>" data-texto3="<?php echo $RecursosCons->RecursosCons['selec_registos']; ?>"></script> 

...
if(isset($_REQUEST["customActionType"]) && $_REQUEST["customActionType"] == "group_action") {
    ...
    //$records["customActionMessage"] = "Group action successfully has been completed. Well done!"; // pass custom message(useful for getting status of group actions)
      ...
      while($row_rsLinguas = $rsLinguas->fetch()) {
        ...
        if($opcao == 1) $query_rsUpd = "UPDATE met_pagamento_".$row_rsLinguas["sufixo"]." SET visivel_footer = '1' WHERE id IN $lista";

        else $query_rsUpd = "UPDATE met_pagamento_".$row_rsLinguas["sufixo"]." SET visivel_footer = '0' WHERE id IN $lista";
  ...
  $colunas = array( 'id', 'nome', 'valor_encomenda', 'valor_encomenda2', '', 'ordem', 'visivel_footer', '');
  ...
  $query_rsTotal = "SELECT * FROM met_pagamento_en WHERE id > '0' AND visivel='1'".$where_pesq.$sOrder;
  ...
  $query_rsTotal = "SELECT * FROM met_pagamento_en WHERE id > '0' AND visivel='1'".$where_pesq.$sOrder." LIMIT $iDisplayStart, $iDisplayLength";
  ...
  while($i < $end && $row_rsTotal = $rsTotal->fetch()) {
    ...
    $nome = utf8_encode($row_rsTotal['nome_interno']);	

    $valor_encomenda = utf8_encode($row_rsTotal['valor_encomenda']);	

    $valor_encomenda2 = utf8_encode($row_rsTotal['valor_encomenda2']);

	  

	  if($valor_encomenda==0) $valor_encomenda = "---";

	  else $valor_encomenda = number_format($valor_encomenda,2,",","")." &pound;";

	  

	  if($valor_encomenda2==0) $valor_encomenda2 = "---";

	  else $valor_encomenda2 = number_format($valor_encomenda2,2,",","")." &pound;";
    ...
    '<span class="label label-sm label-'.$etiqueta.'">'.utf8_encode($visivel).'</span>',
	  ...
	  '<a href="p_met_pagamento-edit.php?id='.$id.'" class="btn btn-xs default btn-editable"><i class="fa fa-pencil"></i> '. $RecursosCons->RecursosCons['btn_editar'].'</a>',

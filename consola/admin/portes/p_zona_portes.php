...
if(isset($_GET['rem']) && $_GET['rem'] == 1) {
	if(isset($_GET['id']) && $_GET['id'] != "" && $_GET['id'] != 0) {
		...
		$query_rsP = "DELETE FROM zonas WHERE id=:id";
		$rsP = DB::getInstance()->prepare($query_rsP);
		$rsP->bindParam(':id', $id, PDO::PARAM_INT);
		$rsP->execute();
		
		$query_rsP = "UPDATE paises SET zona='1' WHERE zona=:id";
		$rsP = DB::getInstance()->prepare($query_rsP);
		$rsP->bindParam(':id', $id, PDO::PARAM_INT);
		$rsP->execute();
		
		$query_rsP = "DELETE FROM zonas_met_envio WHERE id_zona=:id";
		$rsP = DB::getInstance()->prepare($query_rsP);
		$rsP->bindParam(':id', $id, PDO::PARAM_INT);
		$rsP->execute();
		
		$query_rsP = "DELETE FROM zonas_met_pagamento WHERE id_zona=:id";
		$rsP = DB::getInstance()->prepare($query_rsP);
		$rsP->bindParam(':id', $id, PDO::PARAM_INT);
		$rsP->execute();
		...
		if($totalRows_rsExiste>0){
			...
			$query_rsP = "DELETE FROM portes_gratis_zonas WHERE zona=:id";
			$rsP = DB::getInstance()->prepare($query_rsP);
			$rsP->bindParam(':id', $id, PDO::PARAM_INT);
			$rsP->execute();
    ...
    DB::close();
		...
		header("Location: p_zona_portes.php?r=1");
	}
}
        ...
        <div class="col-md-12"> 
          ...
          <div class="portlet">
            <div class="portlet-title">
              ...
              <div class="actions"> <a href="p_zona_portes-insert.php" class="btn default yellow-stripe"> <i class="fa fa-plus"></i> <span class="hidden-480"> <?php echo $RecursosCons->RecursosCons['novo_registo']; ?> </span> </a> </div>
            </div>
...
<script src="p_zona_portes-rpc.js" data-texto1="<?php echo $RecursosCons->RecursosCons['a_carregar']; ?>" data-texto2="<?php echo $RecursosCons->RecursosCons['selec_opcao']; ?>" data-texto3="<?php echo $RecursosCons->RecursosCons['selec_registos']; ?>"></script> 

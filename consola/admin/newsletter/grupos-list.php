...
<?php
  ...
  if(isset($_REQUEST["customActionType"]) && $_REQUEST["customActionType"] == "group_action") {
    ...
    //$records["customActionMessage"] = "Group action successfully has been completed. Well done!"; // pass custom message(useful for getting status of group actions)
		...
		if($opcao == '-1') { // elimina utilizadores selecionados
			$query_rsDel = "UPDATE newsletters_historico SET grupo = 0 WHERE grupo IN $lista";
			$rsDel = DB::getInstance()->query($query_rsDel);
			$rsDel->execute();
			DB::close();	

			$query_rsDel = "DELETE FROM news_grupos WHERE id IN $lista";
			$rsDel = DB::getInstance()->query($query_rsDel);
			$rsDel->execute();
			DB::close();		
		} 
  ...
  $query_rsTotal = "SELECT * FROM news_grupos WHERE id > '0'".$where_pesq.$sOrder;
  ...
  $query_rsTotal = "SELECT * FROM news_grupos WHERE id > '0'".$where_pesq.$sOrder." LIMIT $iDisplayStart, $iDisplayLength";
  $rsTotal = DB::getInstance()->query($query_rsTotal);
  $rsTotal->execute();
  $totalRows_rsTotal = $rsTotal->rowCount();
  DB::close();
  ...
  while($i < $end && $row_rsTotal = $rsTotal->fetch()) {
    ...
    $records["data"][] = array(
	  ...
	  $nome,
	  '<a href="grupos-edit.php?id='.$id.'" class="btn btn-xs default btn-editable"><i class="fa fa-pencil"></i> Editar</a>',
    );

var ConteudoDados = function () {
    ...
    var handleProducts = function() {
		...
		// ler valor passado por data-tema
		var this_js_script = $('script[src*=conteudos-blocos-list]'); // or better regexp to get the file name..
		var id_cont = this_js_script.attr('data-cont'); 
        ...
        grid.init({
            ...
            dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
                ...
                "ajax": {
                    "url": "conteudos-blocos-list.php?id_cont="+id_cont, // ajax source,
                },
        ...
        grid.getTableWrapper().on('click', '.table-group-action-submit', function (e) {
            ...
            if (action.val() != "" && grid.getSelectedRowsCount() > 0) {
                ...
                Metronic.alert({
                    type: 'danger',
                    icon: 'warning',
                    message: 'Sem registos selecionados',
                    container: grid.getTableWrapper(),
                    place: 'prepend'
                });
            }
		...
		$('#bt_submete').click( function() {
			...
			var id_conteudo = document.getElementById('id_conteudo').value;
			
			$.post("conteudos-blocos-rpc.php", {op:"ordenar_registos", campos:data}, function(data){
				window.location='conteudos-blocos.php?id='+id_conteudo;
			});
			return false;
		} );
    }

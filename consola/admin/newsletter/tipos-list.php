...
<?php
  ...
  if(isset($_REQUEST["customActionType"]) && $_REQUEST["customActionType"] == "group_action") {
    ...
    //$records["customActionMessage"] = "Group action successfully has been completed. Well done!"; // pass custom message(useful for getting status of group actions)
		...
		if($opcao == '-1') { // elimina utilizadores selecionados
			$query_rsDel = "UPDATE newsletters SET tipo = 0 WHERE tipo IN $lista";
      ...
      while($row_rsLinguas = $rsLinguas->fetch()) {
  			$query_rsDel = "DELETE FROM news_tipos_".$row_rsLinguas['sufixo']." WHERE id IN $lista";
  			$rsDel = DB::getInstance()->query($query_rsDel);
  			$rsDel->execute();
  			DB::close();	
      }	
  ...
  $query_rsTotal = "SELECT * FROM news_tipos_pt WHERE id > '0'".$where_pesq.$sOrder;
  ...
  $query_rsTotal = "SELECT * FROM news_tipos_pt WHERE id > '0'".$where_pesq.$sOrder." LIMIT $iDisplayStart, $iDisplayLength";
  $rsTotal = DB::getInstance()->query($query_rsTotal);
  $rsTotal->execute();
  $totalRows_rsTotal = $rsTotal->rowCount();
  DB::close();

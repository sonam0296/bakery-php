...
<?php
  ...
  if(CATEGORIAS == 1){
    $query_rsTotal = "SELECT p.*, m.nome as m_nome FROM l_pecas_en p LEFT JOIN l_categorias_en c1 ON c1.id = p.categoria ".$left_join." LEFT JOIN l_marcas_en m ON p.marca = m.id WHERE p.id > '0'".$where_pesq." GROUP BY p.id ".$sOrder;
  }
  else if(CATEGORIAS == 2){
    $query_rsTotal = "SELECT p.*, m.nome as m_nome FROM l_pecas_en p LEFT JOIN l_pecas_categorias pc ON p.id = pc.id_peca LEFT JOIN l_categorias_en c1 ON c1.id = pc.id_categoria ".$left_join." LEFT JOIN l_marcas_en m ON p.marca = m.id WHERE p.id > '0'".$where_pesq." GROUP BY p.id ".$sOrder;
  }
  ...
  if(CATEGORIAS == 1){
    $query_rsTotal = "SELECT p.*, m.nome as m_nome FROM l_pecas_en p LEFT JOIN l_categorias_en c1 ON c1.id = p.categoria ".$left_join." LEFT JOIN l_marcas_en m ON p.marca = m.id WHERE p.id > '0'".$where_pesq." GROUP BY p.id ".$sOrder." LIMIT $iDisplayStart, $iDisplayLength";
  }
  else if(CATEGORIAS == 2){
    $query_rsTotal = "SELECT p.*, m.nome as m_nome FROM l_pecas_en p LEFT JOIN l_pecas_categorias pc ON p.id = pc.id_peca LEFT JOIN l_categorias_en c1 ON c1.id = pc.id_categoria ".$left_join." LEFT JOIN l_marcas_en m ON p.marca = m.id WHERE p.id > '0'".$where_pesq." GROUP BY p.id ".$sOrder." LIMIT $iDisplayStart, $iDisplayLength";
  }
  $rsTotal = DB::getInstance()->query($query_rsTotal);
  $rsTotal->execute();
  $totalRows_rsTotal = $rsTotal->rowCount();

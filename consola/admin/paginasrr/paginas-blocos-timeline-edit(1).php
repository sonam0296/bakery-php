...
$tamanho_imagens1 = getFillSize('Páginas', 'imagem3');
...
if((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "frm_bloco_timeline")) {
	...
	$query_rsP = "SELECT * FROM paginas_blocos_timeline".$extensao." WHERE id=:id";
  ...
  $totalRows_rsLinguas = $rsLinguas->rowCount();
	...
	if($_POST['nome']!='' && $tab_sel == 1) {							
		// actualiza detalhes
		$insertSQL = "UPDATE paginas_blocos_timeline".$extensao." SET nome=:nome, titulo=:titulo, texto=:texto WHERE id=:id";
    ...
    foreach($row_rsLinguas as $linguas) {
      $insertSQL = "UPDATE paginas_blocos_timeline".$extensao." SET ano=:ano WHERE id=:id";
      $rsInsert = DB::getInstance()->prepare($insertSQL);
    ...
    alteraSessions('paginas_fixas');
		...
		if(!$manter) 
			header("Location: paginas-blocos-timeline.php?fixo=".$fixo."&pagina=".$pagina."&id=".$id_bloco."&alt=1");
		else
			header("Location: paginas-blocos-timeline-edit.php?id=".$id."&alt=1&fixo=".$fixo."&pagina=".$pagina."&id_bloco=".$id_bloco."&alt=1&tab_sel=1");
	}
  ...
  if($tab_sel==2) {
    ...
    if(isset($_POST['img_remover1']) && $_POST['img_remover1']==1) {
      if($opcao == 1) {
        $insertSQL = "UPDATE paginas_blocos_timeline".$extensao." SET imagem1=NULL WHERE id=:id";
        $rsInsert = DB::getInstance()->prepare($insertSQL);
        $rsInsert->bindParam(':id', $id, PDO::PARAM_INT, 5);  
        $rsInsert->execute();
        ...
        foreach($row_rsLinguas as $linguas) { 
          $query_rsImagem = "SELECT id FROM paginas_blocos_timeline_".$linguas["sufixo"]." WHERE imagem1=:imagem AND id=:id";
        ...
        foreach($row_rsLinguas as $linguas) {   
          $query_rsSelect = "SELECT imagem1 FROM paginas_blocos_timeline_".$linguas['sufixo']." WHERE id=:id";
          ...
          @unlink('../../../imgs/paginas/'.$row_rsSelect['imagem1']);

          $insertSQL = "UPDATE paginas_blocos_timeline_".$linguas["sufixo"]." SET imagem1=NULL WHERE id=:id";
          $rsInsert = DB::getInstance()->prepare($insertSQL);
    ...
    if($_FILES['img']['name']!='') { // actualiza imagem
      ...
      if($ext != "jpg" && $ext != "jpeg" && $ext != "gif" && $ext != "png") {
        ...
        if($_FILES['img']['name']!='') {
          ...
          if($opcao == 1) {
            $insertSQL = "UPDATE paginas_blocos_timeline".$extensao." SET imagem1=:imagem1 WHERE id=:id";
            $rsInsert = DB::getInstance()->prepare($insertSQL);
          ...
          //Inserir para todas as línguas
            ...
            foreach($row_rsLinguas as $linguas) {   
              $insertSQL = "UPDATE paginas_blocos_timeline_".$linguas["sufixo"]." SET imagem1=:imagem1 WHERE id=:id";
              $rsInsert = DB::getInstance()->prepare($insertSQL);
              $rsInsert->bindParam(':imagem1', $imagem, PDO::PARAM_STR, 5);
              $rsInsert->bindParam(':id', $id, PDO::PARAM_INT, 5);    
              $rsInsert->execute();
    ...
    if($erro == 1)
      header("Location: paginas-blocos-timeline-edit.php?fixo=".$fixo."&pagina=".$pagina."&id_bloco=".$id_bloco."&id=".$id."&erro=1&tab_sel=2");
    else {
      if(!$manter) 
        header("Location: paginas-blocos-timeline.php?fixo=".$fixo."&pagina=".$pagina."&id=".$id_bloco."&alt=1");
      else {
        if($rem == 1 && $ins == 0)
          header("Location: paginas-blocos-timeline-edit.php?fixo=".$fixo."&pagina=".$pagina."&id_bloco=".$id_bloco."&id=".$id."&rem=1&tab_sel=2");
        else
          header("Location: paginas-blocos-timeline-edit.php?fixo=".$fixo."&pagina=".$pagina."&id_bloco=".$id_bloco."&id=".$id."&alt=1&tab_sel=2");
      }
    }
  }
}

$query_rsP = "SELECT * FROM paginas_blocos_timeline".$extensao." WHERE id = :id AND bloco = :id_bloco";
$rsP = DB::getInstance()->prepare($query_rsP);
$rsP->bindParam(':id', $id, PDO::PARAM_INT);
$rsP->bindParam(':id_bloco', $id_bloco, PDO::PARAM_INT);	
$rsP->execute();
$row_rsP = $rsP->fetch(PDO::FETCH_ASSOC);
$totalRows_rsP = $rsP->rowCount();
DB::close();
...
<div class="page-container">
  ...
  <div class="page-content-wrapper">
    <div class="page-content"> 
      <!-- BEGIN PAGE HEADER-->
      <h3 class="page-title"> <?php echo $RecursosCons->RecursosCons['paginas']; ?> <small><?php echo $nome_sel; ?> - <?php echo $RecursosCons->RecursosCons['blocos']; ?> - <?php echo $RecursosCons->RecursosCons['editar_registo']; ?></small> </h3>
      <div class="page-bar">
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-footer">
              <button type="button" class="btn blue" onClick="document.location='paginas-blocos-timeline.php?rem=1&id=<?php echo $row_rsP["id"]; ?>&fixo=<?php echo $fixo; ?>&pagina=<?php echo $pagina; ?>&id=<?php echo $id_bloco; ?>'"><?php echo $RecursosCons->RecursosCons['txt_ok']; ?> </button>
            ...
            <input type="hidden" name="opcao" id="opcao" value="1">
	          <div class="portlet">
	            <div class="portlet-title">
	              <div class="caption"> <i class="fa fa-pencil-square"></i><?php echo $RecursosCons->RecursosCons['blocos_sel_timeline']; ?>  - <?php echo $row_rsP["nome"]; ?> </div>
	              <div class="form-actions actions btn-set">
	                <button type="button" name="back" class="btn default" onClick="document.location='paginas-blocos-timeline.php?fixo=<?php echo $fixo; ?>&pagina=<?php echo $pagina; ?>&id=<?php echo $id_bloco; ?>'"><i class="fa fa-angle-left"></i> <?php echo $RecursosCons->RecursosCons['voltar']; ?></button>
                ...
                <div class="tabbable">
                  <ul class="nav nav-tabs">
                    ...
                    <li <?php if($tab_sel==2) echo "class=\"active\""; ?>> <a href="#tab_imagem" data-toggle="tab" onClick="document.getElementById('tab_sel').value='2'"> <?php echo $RecursosCons->RecursosCons['tab_imagem']; ?> </a> </li>
                  </ul>
                  <div class="tab-content no-space">
                    ...
                    <div class="tab-pane <?php if($tab_sel==2) echo "active"; ?>" id="tab_imagem">
                      <div class="form-body">
                        ...
                        <?php } ?> 
                        <div class="form-group">
                          <label class="col-md-2 control-label" style="text-align:right"><?php echo $RecursosCons->RecursosCons['icone_label']; ?><br>

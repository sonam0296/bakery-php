		...
		if($que_fazer=='order'){		
			...
			if($campo_id>0 && $ordem_final>0){						
				foreach($row_rsLinguas as $linguas) {
					$insertSQL = "UPDATE paginas_".$linguas["sufixo"]." SET ordem=:ordem_final WHERE id=:campo_id";
...
if($_POST['op'] == 'bloco_visivel') {
	$id = $_POST['id'];
	$valor = $_POST['valor'];

  foreach($row_rsLinguas as $linguas) {		
		$insertSQL = "UPDATE paginas_blocos_".$linguas["sufixo"]." SET visivel=:visivel WHERE id=:id";
		$rsInsert = DB::getInstance()->prepare($insertSQL);
		$rsInsert->bindParam(':visivel', $valor, PDO::PARAM_INT);
...
if($_POST['op'] == 'ficheiros_visivel') {
	$id = $_POST['id'];
	$valor = $_POST['valor'];

  foreach($row_rsLinguas as $linguas) {		
		$insertSQL = "UPDATE paginas_blocos_ficheiros_".$linguas["sufixo"]." SET visivel=:visivel WHERE id=:id";
		$rsInsert = DB::getInstance()->prepare($insertSQL);
		$rsInsert->bindParam(':visivel', $valor, PDO::PARAM_INT);
		$rsInsert->bindParam(':id', $id, PDO::PARAM_INT);
		$rsInsert->execute();				
...
if($_POST['op'] == 'galeria_visivel') {	
	...
	$query_rsImgUpdate = "UPDATE paginas_blocos_imgs SET visivel=:visivel WHERE id=:id";

...
{
  $query_rsTotal = "SELECT * FROM encomendas WHERE store_name = '".$store."' AND txn_id != '' OR met_pagamt_id = 9 AND id > '0'".$where_pesq.$sOrder;
...
{
  $query_rsTotal = "SELECT * FROM encomendas WHERE store_name = '".$store."' AND txn_id != '' OR met_pagamt_id = 9 AND id > '0'".$where_pesq.$sOrder." LIMIT $iDisplayStart, $iDisplayLength";
  ...
  while($i < $end && $row_rsTotal = $rsTotal->fetch()) {
    ...
    $records["data"][] = array(
	  ...
	  <a href="imprime_encomenda.php?encomenda='.$id.'" class="btn btn-xs default btn-editable"><i class="fa fa-print"></i>Print</a>
	  ',

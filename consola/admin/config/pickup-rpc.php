...
if($_POST['op'] == 'carregaProdutos') {
  ...
  if(CATEGORIAS == 1) {
    $query_rsProdutos = "SELECT p.id, p.ref, p.nome FROM l_pecas_en p LEFT JOIN l_categorias_en c1 ON c1.id = p.categoria ".$left_join." WHERE p.id > 0 ".$where.$where_cat." GROUP BY p.id ORDER BY p.nome ASC";
    $rsProdutos = DB::getInstance()->prepare($query_rsProdutos);
    $rsProdutos->execute();
    $totalRows_rsProdutos = $rsProdutos->rowCount();
  }
  else if(CATEGORIAS == 2) {
    $query_rsProdutos = "SELECT p.id, p.ref, p.nome FROM l_pecas_en p LEFT JOIN l_pecas_categorias pc ON pc.id_peca = p.id LEFT JOIN l_categorias_en c1 ON c1.id = pc.id_categoria ".$left_join." WHERE p.id > 0 ".$where.$where_cat." GROUP BY p.id ORDER BY p.nome ASC";
    $rsProdutos = DB::getInstance()->prepare($query_rsProdutos);
    $rsProdutos->execute();
    $totalRows_rsProdutos = $rsProdutos->rowCount();
  }

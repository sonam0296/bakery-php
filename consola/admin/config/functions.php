			...
			// this is for create calendra and view add event and view event and number of Events
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $current_Month_First_Day+1 || $current_Month_First_Day == 7) && $cb <= ($total_Days_ofMonthDisplay)){
						...
						$query_rsP = "SELECT title FROM events WHERE date = '".$currentDate."' AND status = 1";
						$rsP = DB::getInstance()->prepare($query_rsP);
						$rsP->execute();
						$result = $rsP->fetchAll();
						$eventNum = $rsP->rowCount();
						DB::close();
/*
...
function get_events_information($date = ''){
	...
	$query_rsP = "SELECT title FROM events WHERE date = '".$date."' AND status = 1";
	$rsP = DB::getInstance()->prepare($query_rsP);
	$rsP->execute();
	$result = $rsP->fetchAll();
	$eventNum = $rsP->rowCount();
...
function add_event_information($date,$title){
	...
	$query_rsInsert = "INSERT INTO events (title,date,created,modified) VALUES ('".$title."','".$date."','".$currentDate."','".$currentDate."')";
	$rsInsert->execute();
/*

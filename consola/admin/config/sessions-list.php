...
<?php
  ...
  $query_rsTotal = "SELECT * FROM config_sessions WHERE id > 0".$where_pesq.$sOrder;
  ...
  $query_rsTotal = "SELECT * FROM config_sessions WHERE id > 0".$where_pesq.$sOrder." LIMIT $iDisplayStart, $iDisplayLength";
  $rsTotal = DB::getInstance()->query($query_rsTotal);
  $rsTotal->execute();
  $totalRows_rsTotal = $rsTotal->rowCount();
  DB::close();
  ...
  while($i < $end && $row_rsTotal = $rsTotal->fetch()) {
    ...
    $records["data"][] = array(
	  $id,
	  $nome,
	  '<a href="sessions-edit.php?id='.$id.'" class="btn btn-xs default btn-editable"><i class="fa fa-pencil"></i> '. $RecursosCons->RecursosCons['btn_editar'].'</a>',
    );

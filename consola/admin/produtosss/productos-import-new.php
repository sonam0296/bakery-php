                   ...
                   continue;  
                    ...
                    if ($all == true) {
                        ...
                        foreach ($row_rsLinguas as $value) {
                           ...
                           $insertSQL = '';
                           $insertSQL = 'INSERT INTO '.$table.' (id,ref, nome, categoria, marca, descricao, composition_per_capsule, modo_de_tomar, detalhes_advertencias, preco, iva, imagem1, imagem2, imagem3, imagem4, visivel, url) VALUES ("'.$i.'", "'.$ref.'", "'.mb_convert_encoding($nome,"HTML-ENTITIES","ISO-8859-1").'", "'.$cate_id.'", "'.$brand_id.'", "'.mb_convert_encoding($descricao,"HTML-ENTITIES","ISO-8859-1").'", "'.mb_convert_encoding($composition_per_capsule,"HTML-ENTITIES","ISO-8859-1").'", "'.mb_convert_encoding($modo_de_tomar,"HTML-ENTITIES","ISO-8859-1").'", "'.mb_convert_encoding($detalhes_advertencias,"HTML-ENTITIES","ISO-8859-1").'", "'.$preco.'",  "'.$iva.'", "'.$imagem.'" , "'.$imagem.'" , "'.$imagem.'", "'.$imagem.'",1,"'.$nome_url.'")';
                           // echo "<pre>";
                           // print_r ($insertSQL);
                           // echo "</pre>";
                           //  exit();

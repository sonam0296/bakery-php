...
<?php
  ...
  $colunas = array( '', 'id', 'nome', 'data_registo', 'pais', 'tipo', 'validado', 'ativo', '');
  ...
  $query_rsTotal = "SELECT id, nome, data_registo, validado, ativo, novo, tipo, pais FROM clientes WHERE id > 0".$where_pesq.$sOrder;
  ...
  $query_rsTotal = "SELECT id, nome, data_registo, validado, ativo, novo, tipo, pais FROM clientes WHERE id > 0".$where_pesq.$sOrder." LIMIT $iDisplayStart, $iDisplayLength";
  $rsTotal = DB::getInstance()->prepare($query_rsTotal);
  $rsTotal->execute();
  $totalRows_rsTotal = $rsTotal->rowCount();

...
<?php
    ...
    //$records["customActionMessage"] = "Group action successfully has been completed. Well done!"; // pass custom message(useful for getting status of group actions)
		...
		if($opcao == '-1') { // elimina utilizadores selecionados
			foreach ($row_rsLinguas as $linguas) {
				$query_rsProc = "SELECT * FROM store_locater WHERE id IN $lista";
				$rsProc = DB::getInstance()->prepare($query_rsProc);
				$rsProc->execute();
				$totalRows_rsProc = $rsProc->rowCount();	

				$query_rsDel = "DELETE FROM store_locater WHERE id IN $lista";
			...
			foreach ($row_rsLinguas as $linguas) {
				if($opcao == 3) $query_rsUpd = "UPDATE store_locater SET visivel = '1' WHERE id IN $lista";
				else $query_rsUpd = "UPDATE store_locater SET visivel = '0' WHERE id IN $lista";
  ...
  $colunas = array( '', 'b_name', 'email', 'ordem', 'visivel', '');
  ...
  if(isset($_REQUEST['action']) && $_REQUEST['action']=="filter") {
		...
		$pesq_email = $_REQUEST['form_email'];
		$pesq_visivel = $_REQUEST['form_visivel'];
		
		if($pesq_form != "") $where_pesq .= " AND (b_name LIKE '%$pesq_form%')";
  ...
  $query_rsTotal = "SELECT * FROM store_locater WHERE id > '0'".$where_pesq.$sOrder;
  ...
  $query_rsTotal = "SELECT * FROM store_locater WHERE id > '0'".$where_pesq.$sOrder." LIMIT $iDisplayStart, $iDisplayLength";
  $rsTotal = DB::getInstance()->prepare($query_rsTotal);
  $rsTotal->execute();
  $totalRows_rsTotal = $rsTotal->rowCount();
  ...
  while($i < $end && $row_rsTotal = $rsTotal->fetch()) {
    ...
    $nome = "";
	  ...
	  $b_name= $row_rsTotal['b_name'];
	  $email = $row_rsTotal['email'];		  
    ...
    $records["data"][] = array(
	  ...
	  '<a href="store-edit.php?id='.$id.'" class="btn btn-xs default btn-editable"><i class="fa fa-pencil"></i> '. $RecursosCons->RecursosCons['editar'].'</a>',
    );

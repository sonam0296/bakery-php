		...
		if($que_fazer=='order') {		
			if($campo_id>0 && $ordem_final>0) {		
				foreach ($row_rsLinguas as $linguas) {			
					$insertSQL = "UPDATE l_categorias_".$linguas["sufixo"]." SET ordem=:ordem_final WHERE id=:campo_id";
			...
			if($campo_id>0) {						
				foreach ($row_rsLinguas as $linguas) {
					if($ordem_final>0){		
						$insertSQL = "UPDATE l_categorias_".$linguas["sufixo"]." SET promocao_desconto=:promocao_desconto WHERE id=:campo_id";
						$rsInsert = DB::getInstance()->prepare($insertSQL);
						$rsInsert->bindParam(':promocao_desconto', $ordem_final, PDO::PARAM_INT);
						$rsInsert->bindParam(':campo_id', $campo_id, PDO::PARAM_INT);
						$rsInsert->execute();
					}else{
						$insertSQL = "UPDATE l_categorias_".$linguas["sufixo"]." SET promocao_desconto=NULL WHERE id=:campo_id";
						$rsInsert = DB::getInstance()->prepare($insertSQL);
						$rsInsert->bindParam(':campo_id', $campo_id, PDO::PARAM_INT);
						$rsInsert->execute();
					}

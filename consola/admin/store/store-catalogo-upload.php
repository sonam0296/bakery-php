...
if (!empty($_FILES)) {
	...
	if($opt == 1) {
		$insertSQL = "UPDATE l_categorias".$extensao." SET catalogo=:imagem WHERE id=:id";
		...
		while($row_rsLinguas = $rsLinguas->fetch()) {
			$insertSQL = "UPDATE l_categorias_".$row_rsLinguas['sufixo']." SET catalogo=:imagem WHERE id=:id";
			$rsInsert = DB::getInstance()->prepare($insertSQL);
			$rsInsert->bindParam(':imagem', $imagem, PDO::PARAM_STR, 5);
			$rsInsert->bindParam(':id', $row_rsP["id"], PDO::PARAM_INT);
			$rsInsert->execute();

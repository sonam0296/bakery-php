...
if((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_cliente")) {
	if($_POST['nome']!='' && $_POST['email']!='' && $_POST['pass']!='' && $_POST['tipo']!='') {
      ...
      if(!$data_nasc) $data_nasc = NULL;		
			...
			$insertSQL = "INSERT INTO clientes (id, tipo, data_registo, email, password, password_salt, data_nasc, nome, pessoa, branch_name, atividade, atividade2, morada, cod_postal, localidade,roll, pais, telefone, telemovel, nif, pvp, desconto, validado, ativo, lingua, cod_bonus) VALUES (:id, :tipo, :data_registo, :email, :password, :password_salt, :data_nasc, :nome, :pessoa, :branch_name, :branch_name, :atividade2, :morada, :cod_postal, :localidade,:roll, :pais, :telefone, :telemovel, :nif, :pvp, :desconto, :validado, :ativo, :lingua, :cod_bonus)";
			$rsInsert = DB::getInstance()->prepare($insertSQL);
      ...
      $rsInsert->bindParam(':roll', $_POST['roll'], PDO::PARAM_STR, 5);
...
$query_rs_branch = "SELECT * FROM branch_list ORDER BY id ASC";
$rsrole_branch = DB::getInstance()->query($query_rs_branch);
$rsrole_branch->execute();
$totalRows_branch = $rsrole_branch->rowCount();
DB::close();
                ...
                <div class="form-body">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-md-2">
                     ...
                     <label class="col-md-2 control-label" for="roll">Role Select: </label>
                    <div class="col-md-2">
                    ...
                    <label class="col-md-2 control-label" for="branch_name">Branch Select: </label>
                    <div class="col-md-2">
                      <select class="form-control" name="branch_name" id="branch_name">
                        <option value=""><?php echo $RecursosCons->RecursosCons['opt_selecionar']; ?></option>
                        <?php if($totalRows_branch > 0) {
                          while($row_rsbranch = $rsrole_branch->fetch()) { ?>
                            <option value="<?php echo $row_rsbranch['branch_name']; ?>"><?php echo $row_rsbranch['branch_name']; ?></option>
                          <?php }
                        } ?>
                      </select>
                    </div>

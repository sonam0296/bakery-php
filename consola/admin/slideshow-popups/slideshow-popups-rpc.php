	...
	foreach ($array as $valor) {
		...
		if($campo_id>0 && $ordem_final>0) {				
			foreach($row_rsLinguas as $linguas) {			
				$insertSQL = "UPDATE banners_popups_".$linguas["sufixo"]." SET ordem=:ordem_final WHERE id=:campo_id";
...
if($_POST['op'] == 'preview_btn') {
	?>	
	<link href='<?php echo ROOTPATH_HTTP; ?>css/modeloBtn.css' rel='stylesheet' type='text/css'>
	<button type="button" class="button <?php echo $_POST['class']?>"><?php echo $RecursosCons->RecursosCons['saiba_mais']; ?></button>
	<?php
}

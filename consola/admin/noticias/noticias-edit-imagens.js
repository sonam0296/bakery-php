var NoticiasEdit = function () {

    var handleImages = function() {
		
		// valores dos atributos passados por parâmetro
		var nome_script = $('script[src*=noticias-edit-imagens]');
        ...
        var uploader = new plupload.Uploader({
            ...
            url : "noticias-edit-imagens-upload.php",
            ...
            init: {
                ...
                FileUploaded: function(up, file, response) {
                    ...
                    if (response.result && response.result == 'OK') {
						...
						$.ajax({
  						  	method: "POST",
						  	url: "noticias-edit-imagens-guardar.php",
  						  	data: { id_noticia: data_id, ficheiro:id }
						}).done(function( data ) {
							console.log("Feito "+data_id);
							$('#uploaded_file_' + file.id + ' > .status').removeClass("label-info").addClass("label-success").html('<i class="fa fa-check"></i> Done'); // set successfull upload
							if(cont2==cont) document.location='noticias-edit-imagens.php?id='+data_id+'&suc=1';
						});
                    } else {

...
class SMTP extends MIME {
		...
		if(is_string($conn)){
			...
			if(FUNC::is_alpha($conn, false, '-')){
				...
				foreach($exp as $val){
					...
					else trigger_error('Invalid connection type value "'.$val.'", on class SMTP::delivery()', 512);
				}
				if(count($rep) > 0){
					$this->_smtpconn = $rep;
					$ret = true;
				}
			}else trigger_error('Invalid parameter value, on class SMTP::delivery()', 512);
		}else trigger_error('Invalid parameter type value, on class SMTP::delivery()', 512);
		return $ret;
	...
	function port($num){
		...
		if(is_int($num)){
			$this->_port = $num;
			$ret = true;
		}else trigger_error('Invalid parameter type value, on class SMTP::port()', 512);
		return $ret;
	...
	function timeout($num){
		...
		if(is_int($num)){
			$this->_timeout = $num;
			$ret = true;
		}else trigger_error('Invalid parameter type value, on class SMTP::timeout()', 512);
		return $ret;
		...
		if(is_string($raddr)){
			...
			if($raddr != ""){
				...
				if(FUNC::is_ipv4($raddr)) $rvip = $raddr;
					...
					if($rvip == $raddr){
						$ret = false;
						trigger_error('Invalid hostname value "'.$raddr.'", on class SMTP::relay()', 512);
					}
				}
			}else trigger_error('Invalid hostname/ip value, on class SMTP::relay()', 512);
		}else trigger_error('Invalid hostname/ip type value, on class SMTP::relay()', 512);
		if($ret){
			if(is_bool($ruser)){
				if(!$ruser) $ruser = '';
				else trigger_error('Invalid username 1 type value, on class SMTP::relay()', 512);
			}elseif(is_string($ruser)){
				$ruser = FUNC::str_clear($ruser);
				$ruser = trim($ruser);
			}else{
				$ruser = '';
				trigger_error('Invalid username 2 type value, on class SMTP::relay()', 512);
			}
			if(is_bool($rpass)){
				if(!$rpass) $rpass = '';
				else trigger_error('Invalid password 1 type value, on class SMTP::relay()', 512);
			}elseif(is_string($rpass)){
				$rpass = FUNC::str_clear($rpass);
				$rpass = trim($rpass);
			}else{
				$rpass = '';
				trigger_error('Invalid password type value, on class SMTP::relay()', 512);
			}
			if(($ruser != "" && $rpass == "") || ($ruser == "" && $rpass != "")){
				$ruser = $rpass = '';
				trigger_error('Invalid username and password combination value, on class SMTP::relay()', 512);
			}
			if(!is_int($rport)){
				$rport = 25;
				trigger_error('Invalid port type value, on class SMTP::relay()', 512);
			}
			if(is_string($rauth)){
				$rauth = trim(strtolower($rauth));
				if(!($rauth == "autodetect" || $rauth == "login" || $rauth == "plain")){
					$rauth = 'autodetect';
					trigger_error('Invalid auth value, on class SMTP::relay()', 512);
				}
			}else{
				$rauth = 'autodetect';
				trigger_error('Invalid auth type value, on class SMTP::relay()', 512);
			}
			if(is_string($rvssl)){
				...
				if(!($rvssl == "tls" || $rvssl == "ssl")){
					$rvssl = false;
					trigger_error('Invalid TLS/SSL value, on class SMTP::relay()', 512);
				}
			}else{
				if(is_bool($rvssl)){
					...
					$rvssl = false;
					trigger_error('Invalid TLS/SSL type value, on class SMTP::relay()', 512);
				}
			}
			...
			if($hname != ""){
				if(is_string($hvalue)){
					...
					if($hvalue != ""){
						...
						if($vname == "subject") trigger_error('Can not set "Subject" header value, for this, use function "Send()", on class SMTP::addheader()', 512);
						elseif($vname == "from") trigger_error('Can not set "From" header value, for this, use function "From()", on class SMTP::addheader()', 512);
						elseif($vname == "to") trigger_error('Can not set "To" header value, for this, use function "AddTo()", on class SMTP::addheader()', 512);
						elseif($vname == "cc") trigger_error('Can not set "Cc" header value, for this, use function "AddCc()", on class SMTP::addheader()', 512);
						elseif($vname == "bcc") trigger_error('Can not set "Bcc" header value, for this, use function "AddBcc()", on class SMTP::addheader()', 512);
						elseif($vname == "date") trigger_error('Can not set "Date" header value, this value is automaticaly set, on class SMTP::addheader()', 512);
						elseif($vname == "x-mailer") trigger_error('Can not set "X-Mailer" header value, this value is automaticaly set, on class SMTP::addheader()', 512);
						elseif($vname == "content-type") trigger_error('Can not set "Content-Type" header value, this value is automaticaly set, on class SMTP::addheader()', 512);
						elseif($vname == "content-transfer-encoding") trigger_error('Can not set "Content-Transfer-Encoding" header value, this value is automaticaly set, on class SMTP::addheader()', 512);
						elseif($vname == "content-disposition") trigger_error('Can not set "Content-Disposition" header value, this value is automaticaly set, on class SMTP::addheader()', 512);
						elseif($vname == "x-priority") trigger_error('Can not set "X-Priority" header value, for this, use function "Priority()", on class SMTP::addheader()', 512);
						elseif($vname == "x-msmail-priority") trigger_error('Can not set "X-MSMail-Priority" header value, for this, use function "Priority()", on class SMTP::addheader()', 512);
						elseif($vname == "mime-version") trigger_error('Can not set "MIME-Version" header value, this value is automaticaly set, on class SMTP::addheader()', 512);
						else{
							$ret = true;
							$this->_header[] = array('name' => ucfirst($hname), 'value' => $hvalue);
						}
					}else trigger_error('Invalid 2\'nd parameter value, on class SMTP::addheader()', 512);
				}else trigger_error('Invalid 2\'nd parameter type value, on class SMTP::addheader()', 512);
			}else trigger_error('Invalid 1\'st parameter value, on class SMTP::addheader()', 512);
		}else trigger_error('Invalid 1\'st parameter type value, on class SMTP::addheader()', 512);
		return $ret;
	...
	function delheader($hname){
		...
		if(is_string($hname)){
			...
			}else trigger_error('Invalid parameter value, on class SMTP::delheader()', 512);
		}else trigger_error('Invalid parameter type value, on class SMTP::delheader()', 512);
		return $ret;
		...
		if(is_string($adrr)){
			...
			if($adrr != "" && FUNC::is_mail($adrr)){
				if(!isset($this->_toaddrs[$adrr])){
					...
					}else trigger_error('Invalid 2\'nd parameter type value, on class SMTP::addto()', 512);
				}else trigger_error('Already exists, on class SMTP::addto()', 512);
			}else trigger_error('Invalid 1\'st parameter value, on class SMTP::addto()', 512);
		}else trigger_error('Invalid 1\'st parameter type value, on class SMTP::addto()', 512);
		return $ret;
	...
	function delto($adrr = 'all'){
		...
		if(is_string($adrr)){
			...
			if($adrr != ""){
				...
				}else trigger_error('Invalid 2 parameter value, on class SMTP::delto()', 512);
			}else trigger_error('Invalid 1 parameter value, on class SMTP::delto()', 512);
		}else trigger_error('Invalid parameter type value, on class SMTP::delto()', 512);
		return $ret;
		...
		if(is_string($adrr)){
			...
			if($adrr != "" && FUNC::is_mail($adrr)){
				if(!isset($this->_ccaddrs[$adrr])){
					...
					}else trigger_error('Invalid 2\'nd parameter type value, on class SMTP::addcc()', 512);
				}else trigger_error('Already exists, on class SMTP::addcc()', 512);
			}else trigger_error('Invalid 1\'st parameter value, on class SMTP::addcc()', 512);
		}else trigger_error('Invalid 1\'st parameter type value, on class SMTP::addcc()', 512);
		return $ret;
	...
	function delcc($adrr = 'all'){
		...
		if(is_string($adrr)){
			...
			if($adrr != ""){
				...
				}else trigger_error('Invalid 2 parameter value, on class SMTP::delcc()', 512);
			}else trigger_error('Invalid 1 parameter value, on class SMTP::delcc()', 512);
		}else trigger_error('Invalid parameter type value, on class SMTP::delcc()', 512);
		return $ret;
		...
		if(is_string($adrr)){
			...
			if($adrr != "" && FUNC::is_mail($adrr)){
				if(!isset($this->_bccaddrs[$adrr])){
					$this->_bccaddrs[$adrr] = '';
					$ret = true;
				}else trigger_error('Already exists, on class SMTP::addbcc()', 512);
			}else trigger_error('Invalid parameter value, on class SMTP::addbcc()', 512);
		}else trigger_error('Invalid parameter type value, on class SMTP::addbcc()', 512);
		return $ret;
	...
	function delbcc($adrr = 'all'){
		...
		if(is_string($adrr)){
			...
			if($adrr != ""){
				...
				}else trigger_error('Invalid 2 parameter value, on class SMTP::delbcc()', 512);
			}else trigger_error('Invalid 1 parameter value, on class SMTP::delbcc()', 512);
		}else trigger_error('Invalid parameter type value, on class SMTP::delbcc()', 512);
		return $ret;
	...
	function from($adrr, $name = ''){
		...
		if(is_string($adrr)){
			...
			if($adrr != "" && FUNC::is_mail($adrr)){
				...
				}else trigger_error('Invalid 2\'nd parameter type value, on class SMTP::from()', 512);
			}else trigger_error('Invalid mail address format, on class SMTP::from()', 512);
		}else trigger_error('Invalid first parameter type value, on class SMTP::from()', 512);
		return $ret;
	...
	function fromhost($server, &$havemx){
		...
		if(is_string($server)){
			...
			}else trigger_error('Invalid parameter value, on class SMTP::fromhost()', 512);
		}else trigger_error('Invalid parameter type value, on class SMTP::fromhost()', 512);
		return $ret;

	}

	function text($text, $charset = 'UTF-8', $encoding = 'quoted-printable', $disposition = 'inline'){
		...
		if(is_string($charset)){
			...
			if(!($charlen > 1 && $charlen < 60)){
				$charset = 'us-ascii';
				trigger_error('Invalid charset value, on class SMTP::text()', 512);
			}
		}else{
			$charset = 'us-ascii';
			trigger_error('Invalid charset type value, on class SMTP::text()', 512);
		}
		if(is_string($encoding)){
			...
			if(!($encoding != "" && isset($this->_arrenc[$encoding]))){
				...
				trigger_error('Invalid encoding value, on class SMTP::text()', 512);
			}
		}else{
			$encoding = 'quoted-printable';
			trigger_error('Invalid encoding type value, on class SMTP::text()', 512);
		}
		if(is_string($disposition)){
			...
			if(!($disposition == "attachment" || $disposition == "inline")){
				$disposition = 'inline';
				trigger_error('Invalid disposition value, on class SMTP::text()', 512);
			}
		}else{
			$disposition = 'inline';
			trigger_error('Invalid disposition type value, on class SMTP::text()', 512);
		}
		if(is_string($text)){
			$text = trim($text);
			if($text == ""){
				...
				$ret = true;
			}else trigger_error('Invalid text/plain value, on class SMTP::text()', 512);
		}else trigger_error('Invalid text/plain type value, on class SMTP::text()', 512);
		return $ret;

	}

	function html($html, $charset = 'UTF-8', $encoding = 'quoted-printable', $disposition = 'inline'){
		...
		if(is_string($charset)){
			...
			if(!($charlen > 1 && $charlen < 60)){
				$charset = 'us-ascii';
				trigger_error('Invalid charset value, on class SMTP::html()', 512);
			}
		}else{
			$charset = 'us-ascii';
			trigger_error('Invalid charset type value, on class SMTP::html()', 512);
		}
		if(is_string($encoding)){
			$encoding = FUNC::str_clear($encoding, array(' '));
			$encoding = trim(strtolower($encoding));
			if(!($encoding != "" && isset($this->_arrenc[$encoding]))){
				$encoding = 'quoted-printable';
				trigger_error('Invalid encoding value, on class SMTP::html()', 512);
			}
		}else{
			$encoding = 'quoted-printable';
			trigger_error('Invalid encoding type value, on class SMTP::html()', 512);
		}
		if(is_string($disposition)){
			$disposition = FUNC::str_clear($disposition, array(' '));
			$disposition = trim(strtolower($disposition));
			if(!($disposition == "attachment" || $disposition == "inline")){
				$disposition = 'inline';
				trigger_error('Invalid disposition value, on class SMTP::html()', 512);
			}
		}else{
			$disposition = 'inline';
			trigger_error('Invalid disposition type value, on class SMTP::html()', 512);
		}
		if(is_string($html)){
			$html = trim($html);
			if($html != ""){
				...
				$ret = true;
			}else trigger_error('Invalid text/html value, on class SMTP::html()', 512);
		}else trigger_error('Invalid text/html type value, on class SMTP::html()', 512);
		return $ret;
			...
			if(is_string($name)){
				...
				if($name != ""){
					...
					if(is_string($disposition)){
						...
						else trigger_error('Invalid disposition value, on class SMTP::attachsource()', 512);
					}else trigger_error('Invalid disposition type value, on class SMTP::attachsource()', 512);
					$encode = 'base64';
					if(is_string($encoding)){
						$encoding = FUNC::str_clear($encoding, array(' '));
						$encoding = trim(strtolower($encoding));
						if($encoding != "" && isset($this->_arrenc[$encoding])) $encode = $encoding;
						else trigger_error('Invalid encoding value, on class SMTP::attachsource()', 512);
					}else trigger_error('Invalid encoding type value, on class SMTP::attachsource()', 512);
					$this->_attach[] = array('name' => $name, 'mime' => $mime, 'disp' => $disp, 'encode' => $encode, 'source' => $source);
				}else trigger_error('Invalid name value, on class SMTP::attachsource()', 512);
			}else trigger_error('Invalid name type value, on class SMTP::attachsource()', 512);
		}else trigger_error('Invalid source value, on class SMTP::attachsource()', 512);
		return $ret;
	...
	function attachfile($file, $name = false, $mimetype = 'autodetect', $disposition = 'attachment', $encoding = 'base64'){
		...
		if(is_string($file)){
			...
			}else trigger_error('Invalid file source, on class SMTP::attachfile()', 512);
		}else trigger_error('Invalid file type value, on class SMTP::attachfile()', 512);
		return $ret;
	...
	function delattach($name = true){
		...
		if(is_bool($name)){
			if($name){
				$this->_attach = false;
				$ret = true;
			}else trigger_error('Invalid 2 file name type value, on class SMTP::delattach()', 512);
			...
			}else trigger_error('Invalid file name value, on class SMTP::delattach()', 512);
		}else trigger_error('Invalid file name type value, on class SMTP::delattach()', 512);
		return $ret;
	...
	function priority($level = 3){
		...
		if(is_int($level)){
			...
			else trigger_error('Invalid 1 parameter value, on class SMTP::priority()', 512);
			...
			else trigger_error('Invalid 2 parameter value, on class SMTP::priority()', 512);
		}else trigger_error('Invalid parameter type value, on class SMTP::priority()', 512);
		if($set){
		...
		if($hname == "localhost"){
			...
			if($isrelay) $ret = $this->_sendtoip($hname, $arrto, $isrelay);
				...
				if(FUNC::is_ipv4($hname)) $ret = $this->_sendtoip($hname, $arrto, $isrelay);
					...
					}else trigger_error('Can not find any valid ip address for hostname "'.$hname.'", on class SMTP::_sendtohost()', 512);
				}
			}
		}
		return $ret;
	...
	function _splitmsg($longmsg, $approx = 10240){
		...
		//DAVIDE » Problema com o quoted-printable nos "." quando fazia quebra de linha || 28/03/2018
		...
		if(is_string($subject)){
			...
			if($subject != ""){
				...
				if(!($this->_toaddrs && count($this->_toaddrs) > 0)){
					$ver = false;
					trigger_error('You must set "To" e-mail address(es) using function "AddTo()", on class SMTP::send()', 512);
				}
				if(!($this->_atext || $this->_ahtml)){
					$ver = false;
					trigger_error('You must set the mail message using function(s) "Text() or/and Html()", on class SMTP::send()', 512);
				}
				if($ver){
					if($this->_ccaddrs && count($this->_ccaddrs) > 0){
						$clearcc1 = array();
						foreach($this->_ccaddrs as $ccaddrs1 => $ccname1){
							...
							else trigger_error('The e-mail address "'.$ccaddrs1.'" appear in To and Cc, on class SMTP::send()', 512);
						}
					...
					if($this->_bccaddrs && count($this->_bccaddrs) > 0){
						...
						foreach($this->_bccaddrs as $bccaddrs1 => $bccname1){
							...
							else trigger_error('The e-mail address "'.$bccaddrs1.'" appear in To and Bcc, on class SMTP::send()', 512);
						}
					...
					if($this->_bccaddrs && count($this->_bccaddrs) > 0 && $this->_ccaddrs && count($this->_ccaddrs) > 0){
						...
						foreach($this->_bccaddrs as $bccaddrs2 => $bccname2){
							...
							else trigger_error('The e-mail address "'.$bccaddrs2.'" appear in Cc and Bcc, on class SMTP::send()', 512);
						}
			...
			}else trigger_error('Invalid subject value, on class SMTP::send()', 512);
		}else trigger_error('Invalid subject type value, on class SMTP::send()', 512);
		return $ret;

...
if(ECOMMERCE_ATIVO == 0) {
 ...
 $cartvaluecheck = $class_carrinho->precoTotal(); 
 if($cartvaluecheck <= 0)
 {
   header("Location: ".ROOTPATH_HTTP."carrinho.php?errorcart=1");
...
/*$query_rsCar = "SELECT pecas.id, pecas.nome, pecas.prepare, pecas.ref, pecas.url, pecas.nao_limitar_stock, carrinho.*, carrinho.id AS id_linha FROM carrinho LEFT JOIN l_pecas_en AS pecas ON (carrinho.produto = pecas.id AND pecas.visivel = 1) WHERE carrinho.session = '$carrinho_session' ORDER BY pecas.ordem ASC";
$rsCar = DB::getInstance()->prepare($query_rsCar);
$rsCar->execute();
                ...
                <form action="carrinho-comprar2.php" id="comprar" name="comprar" method="post" onSubmit="return validaPasso()" autocomplete="off" novalidate>   
                  <div class="row collapse align-strecht">
                    <div class="small-12 medium-8 column">
                          ...
                          <div class="carrinho_modal" id="form_envio">
                            <div class="modal">
                              ...
                              <div class="div_100 edit_modal">                                                            
                                ...
                                <input class="carrinho_inpt" placeholder="<?php echo $Recursos->Resources["comprar_contacto"]; ?> *" type="text" name="contacto" id="contacto" value="<?php echo $row_rsCliente['telemovel']; ?>" />
                                ...
                                <button type="button" name="savedata" onClick="guarda_envio()"><span><?php echo $Recursos->Resources["comprar_guardar"]; ?></span></button>
                              </div>
                            </div>
                          </div>
                        </div>
                     ...
                     <?php $total_str = $class_carrinho->precoTotal(); ?>
                    <div class="small-12 medium-4 column">
                      <div class="pickup_delivery comprar_resumo">
                        <div class="pickup_deliveryinner">
                            <h2>pickup delivery</h2>
                            <div class="pickup_delivery_box">
                              <div class="dp_redio_btn">
                                <?php if($total_str >= 100) { ?>
                                  <div class="dp_redio_box radio-select">
                                    <input type="radio" required id="delivery" name="delivery" value="1" >
                                    <label for="delivery">Delivery</label>
                                  </div><!-- /dp_redio_box -->
                                <?php } ?>
                                  <div class="dp_redio_box radio-select">
                                    <input type="radio" required id="pickup" name="delivery" value="0">
                                ...
                                <div id= "hidepickup">
                                  ...
                                  <input type="text"  name="data" class="<?php if($row_rsP_Event["start_event"] == $date) { echo "red"; } ?>"   id="datepicker"  value="<?php if($_POST["data"] != "") { echo $_POST["date"]; } else { } ?>" onchange="datechanged(this)" readonly="readonly">     
                                 
                                  <a  onclick="document.getElementById('datepicker').value = ''">clear</a>
                                </div>
                               </div><!-- /pickup_delivery_date -->
                            </div><!-- /pickup_delivery_box -->
                        </div><!-- /pickup_deliveryinner -->
                  ...
                  <div class="row collapse buttons_marg">
                    <div class="small-12 medium-8 column">
                      <button class="carrinho_btn icon_carrinho" name="pickupdate" type="submit"><span style="position: relative;"><?php echo $Recursos->Resources["comprar_seguinte"]." <span class='total_update'>".$total_str."</span>"; ?> <i class="icon-right-carrinho"></i></span></button>
                    ...
                    <?php $subtotal = $class_carrinho->mostraPreco($total, 2);  ?>
                  <input type="hidden" name="total_inpt" id="total_inpt" value="<?php echo $class_carrinho->mostraPreco($subtotal, 2); ?>" />
...
function validaPasso() {
  ...
  if(document.getElementById('contacto')) {
    _telefone = document.getElementById('contacto');
  }
  if(document.getElementsByName('delivery')) {
    _pickup = document.getElementsByName('delivery')[0].value;
  }
...
function precoPortes(tipo, id) {
  ...
  /*Shipping Condition Code Start || Added Vishal Prajapti || 03-nov-2020*/

   var preco = $('#total_inpt').val();
   var port =   $('#entrega_'+id).val();

    if(preco <= 100 && port == 11)
    {
     portes = $('#entrega_'+id).attr('data-preco');
    }
    else if(preco <= 50 && port == 13)
    {
       portes = $('#entrega_'+id).attr('data-preco');  
    }
    else if(port != 11 && port != 13)
    {
      portes = $('#entrega_'+id).attr('data-preco');  
    }
...
carrega_metodos();
  ...
  <script type="text/javascript">
    function deliverypick() {
      var dilcheck = $('input[name="delivery"]:checked').val();
      var entrega_hide = $('#entrega_8');
      if (dilcheck == '1') {
        $('#hidepickup').hide();
        entrega_hide.parent('.pagamentos_divs').siblings('.pagamentos_divs').show();
        entrega_hide.parent('.pagamentos_divs').hide();
      }else{
        $('#hidepickup').show();
        entrega_hide.parent('.pagamentos_divs').siblings('.pagamentos_divs').hide();
        entrega_hide.parent('.pagamentos_divs').show();
      }
    }
    ...
    jQuery(document).ready(function($) {
      $('#hidepickup').hide();
       setTimeout(function() {
         $('#entrega_8').parent('.pagamentos_divs').hide();
       }, 500);
      deliverypick();
    });

    $(document).on('click', '.radio-select', function(event) {
      event.preventDefault();
      $(this).siblings('.radio-select').find('input[name="delivery"]').removeAttr('checked');
      $(this).find('input[name="delivery"]').attr('checked', 'checked');;
      deliverypick();
    });
  </script>
 <script>
...
<script type="text/javascript">
 ...
 <script type="text/javascript">
  $(".radio-select").click(function() {
    //console.log('ddd');
    setTimeout(function() {
    var checked = $('input[name="delivery"]:checked').val();
    //console.log(checked)
      if ( checked == 0){
        console.log('dds');

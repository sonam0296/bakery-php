...
if($_POST['op'] == 'datechecker') {
	...
	$query_rsP_Event = "SELECT * FROM events Where start_event =  '".$newDate."' AND visivel = 1";
	//echo $query_rsP_Event;
	...
	if($totalRows_rsP_Event > 0)
	{
		$get_date = array(
			"status" => "1",
			"lable"  =>  $row_rsP_Event["title"],
			"holiday"=> $row_rsP_Event["start_event"]
		);
	}
	else {
		$get_date = array(
		...
		if($stock_disp > 0 && $stock_disp >= $qtd || $row_rsP['nao_limitar_stock'] == 1) {
			if($totalRows_rsProc > 0 && $row_rsP["tem_conjunto"] != 1) {
				...
				if($totalRows_rsOferta > 0) {	
					...
					$insertSQL = "INSERT INTO carrinho (id, id_oferta, session, produto, opcoes, quantidade, preco,message, op1, op2, op3, op4, op5) VALUES ('$id2', '$id', '$carrinho_session', '".$row_rsOferta["id_oferta"]."', '$message', '', '$quantidade', '0', '0', '0', '0', '0', '0')";
					$rsInsertSQL = DB::getInstance()->prepare($insertSQL);
					$rsInsertSQL->execute();
				}
			}	
...
if($_POST['op'] == "carregaPagamentos") {		
	...
	if($valor_pagamt > 0) { 
         ...
         ?> 		   	
		   	...
		   	<?php if($totalRows_rsRole["roll_name"] == "customer" && $verifica && $row_rsQtds['id'] == 10 && $valor_pagamt > $totalRows_rsRole["cod_price"]) { ?>
     ...
     ?>
    	...
    	if($totalRows_rsRole["roll_name"] == "franchise" && $verifica && $row_rsQtds['id'] == 10){ echo  $totalRows_rsRole["roll_name"];?>
    		<div class="div_100 pagamentos_divs">
		    	<input required name="pagamento" type="radio" value="<?php echo $row_rsQtds['id']; ?>" id="pagamento_<?php echo $row_rsQtds['id']; ?>" onclick="verifica_pag(this.value)" <?php if($met_pagamento == $row_rsQtds['id'] || $totalRows_rsQtds == 1) echo "checked=\"checked\""; ?> data-preco="<?php echo $preco_pag; ?>" />
	        <label for="pagamento_<?php echo $row_rsQtds['id'];?>">
            <div class="img"><img src="<?php echo $img; ?>" width="100%" /></div><!--
...
if($_POST['op'] == "carregaEntregas") {
	...
	foreach ($row_rsQTDTemprsQtdsNew as $row_rsQTDTemp) {
    	...
    	//while($row_rsQtds = $rsQtdsNew->fetch()) {
			...
			if(time() > strtotime('2 pm') || time() < strtotime('3 pm')) {
			  $mile_price = 2;
			} elseif(time() > strtotime('4 am') || time() < strtotime('7 pm')) {
			  $mile_price = 1;
			}
			...
			if( $row_rsQtds['id'] == 14 && $_POST['miless'] > $check_miles){
				...
				//echo $mile_price;
				//$preco_env = $mile_price;
				$preco_env = ($mile_price *  $_POST['miless']) - $check_miles;
			}
			if( $row_rsQtds['id'] == 14 && $_POST['miless'] < $check_miles){
				$preco_env = $preco_pag;
			}
		...
		<div class="small-12 column">
	      ...
	      <div class="small-12 medium-6 column carrinho_btn_continuar">
	        ...
	        <?php } else if($row_rsCliente > 0) { ?>
	        	<a class="store_name_car carrinho_btn icon_carrinho" href="carrinho-comprar.php"><span style="position: relative;"><?php echo $Recursos->Resources["carrinho_btn_next"]; ?> <i class="icon-right-carrinho"></i></span></a>
	        <?php } else { ?>
